...
# ВСего скорей пользователи будут их искать по имени и надо будет 
...
# По имени файла никто искать не будет (так как имя то будет переделано на UID какой-нить

# profiles
# Вот тут можно больше разгуляться

# Я ба сделал вот такой, предполагая что ищут по дате рождения, и возможно полу
...
# а если у проекта есть ярко выраженные пики загрузки и простоя то и пересчет не составляет проблем 
...
# Сначала подумал что JOIN likes ON media.id = likes.target_id не правильно будет так как 
...
# смотрим медиа и пользователя с самым большим кол-вом лайков  
...
# То что касается этого запроса то самое его узкое место на мой взгляд состоит в том, что мы джоиним каждый раз лишнюю информацию
# и что-то мне подсказывает что тут даже не делай ничего с нормализацией можно будет сильно ускорится
...
# А то что касается исправления базы данных то тут их конечно много разных можно придумать...
# И отдельное поле для всех лайков на всё что есть, и по полю для каждого типа target и может поле JSON куда писать лайки в виде {<media.id>:<total_likes>,<media.id>:<total_likes>}
...
# 3.7 И среднее кол-во пользователей в группах 
# У меня не хватило сил это посчитать, надо хорошо потренироваться с ними, уже просто так не понимаю куда тыкаться
...
# 3.7 Объявил окна для наглядности, можно наверное и 2-мя обойтись но как раз интересно будет посмотреть скажется ли это на производительности  
SELECT DISTINCT communities.id, communities.name,
  (SELECT count(*) FROM communities) as averag_users,
